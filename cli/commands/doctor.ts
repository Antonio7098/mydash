...
import type { ReadinessCheck } from "../../src/workspace/types.js";
...
export const doctorCommand: CommandDefinition = {
  ...
  async run(invocation, context) {
    ...
    const requestedRoot = typeof invocation.options.workspace === "string"
...
function checkNode(): ReadinessCheck {
...
function checkLibreOffice(): ReadinessCheck {
...
async function inspectWorkspace(startPath: string): Promise<{
  root: string | null;
  user: string | null;
  ...
  if (!root) {
    ...
    return { root: null, user: null, checks, warnings };
  }
  ...
  try {
    ...
    checks.push({
      ...
      status: "pass",
      value: config.user,
      message: `Artifact commands are scoped to ${config.user}.`,
    });
  } catch (error) {
    checks.push({
      id: "workspace-config",
  ...
  return { root, user, checks, warnings };
}
...
function deriveCapabilities(checks: readonly ReadinessCheck[]) {
