...
import { inspectDataset, type InspectDatasetResult } from "../../src/data/inspect.js";
import { profileDataset, type ProfileDatasetResult } from "../../src/data/profile.js";
import {
  deduplicateRecords,
  filterRecords,
  parseColumnList,
...
import { writeDataset } from "../../src/data/write.js";
import {
  createRecipe,
  refreshRecipe,
...
import {
  ...
  stageArtifactSource,
  syncArtifactSource,
} from "../../src/data/artifact-refresh.js";
...
async function runProfile(
  ...
  const data = await profileDataset(path, {
    ...
    maxRows,
    topValues,
  } as unknown as Parameters<typeof profileDataset>[1]);
...
async function runCreateRecipe(
  args: readonly string[],
  context: CommandContext,
  workspaceRoot: string | undefined,
) {
  ...
  const requiredOptions = parsed.options as unknown as typeof parsed.options &
    Record<"id" | "output" | "recipe", string>;
  ...
  const result = await createRecipe({
    workspaceRoot,
    sourcePath: source,
    recipePath: resolve(workspaceRoot, requiredOptions.recipe),
    ...
    outputPath: requiredOptions.output,
...
async function runSync(
  ...
  const data = await syncArtifactSource({
    ...
    force: parsed.options.force ?? false,
    history: !(parsed.options.noHistory ?? false),
  } as unknown as Parameters<typeof syncArtifactSource>[0]);
...
async function runRefreshArtifact(
  args: readonly string[],
...
function requireNamed<
  const Options extends Record<string, string | boolean | undefined>,
  const Names extends readonly string[],
>(
  options: Options,
  names: Names,
  operation: string,
): asserts options is Options & Record<Names[number], string> {
...
function renderInspection(data: InspectDatasetResult): string {
  return [
...
function renderProfile(data: ProfileDatasetResult): string {
  const lines = [
