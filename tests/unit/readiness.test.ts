...
import { buildReadinessReport, unavailableGit } from "../../src/workspace/readiness.js";
...
function input(overrides = {}) {
  return {
    ...
    git: { available: true, branch: "main", summary: { conflicted: 0 }, identity: { configured: true }, remotes: ["origin"] },
...
test("Git is optional for browsing and export", () => {
  ...
  assert.equal(report.status, "ready");
  const gitCheck = report.checks.find((item) => item.id === "git");
  if (!gitCheck) throw new Error("git check not found");
  assert.equal(gitCheck.state, "warning");
});
...
test("required validation failure blocks readiness", () => {
  ...
  if (!report.nextActions[0]) throw new Error("expected next actions");
