...
test("appearance query parsing validates JSON and size", () => {
  ...
  const parsed = parseAppearanceQuery(source);

  if (!parsed) throw new Error("Expected parsed appearance query.");

  assert.equal(parsed.theme, "core/hsbc-light");
  assert.throws(() => parseAppearanceQuery("{bad"), /valid JSON/);
  assert.throws(
    () => parseAppearanceQuery(source, 5),
    /exceeds 5 bytes/,
  );
});
...
test("applying appearance does not mutate the original manifest", () => {
  ...
  const nextAppearance = next.manifest.appearance;

  if (
    !nextAppearance ||
    typeof nextAppearance !== "object" ||
    !("theme" in nextAppearance)
  ) {
    throw new Error("Expected normalised appearance.");
  }

  assert.equal(artifact.manifest.appearance.theme, "old");
  assert.equal(nextAppearance.theme, "core/hsbc-light");
...
test("appearance options canonicalise current references for select controls", () => {
  ...
  const summaryOptions = result.options.components.summary;

  if (!summaryOptions) throw new Error("Expected summary component options.");

  assert.deepEqual(
    summaryOptions.map((item) => item.reference),
