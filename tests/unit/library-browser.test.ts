...
test("library filters search semantic metadata", () => {
  assert.deepEqual(filterLibraryEntries(entries, { query: "accessible" }).map((entry: { id: string }) => entry.id), ["button"]);
  assert.deepEqual(filterLibraryEntries(entries, { level: "local" }).map((entry: { id: string }) => entry.id), ["governance-pipeline"]);
  assert.deepEqual(filterLibraryEntries(entries, { slot: "metric-summary" }).map((entry: { id: string }) => entry.id), ["metric-card"]);
  assert.deepEqual(filterLibraryEntries(entries, { kind: ["component", "primitive"] }).map((entry: { id: string }) => entry.id), ["metric-card", "button", "governance-pipeline"]);
  assert.deepEqual(filterLibraryEntries(entries, { slot: ["button", "governance-pipeline"] }).map((entry: { id: string }) => entry.id), ["button", "governance-pipeline"]);
});

test("library sorting prefers Core before Local", () => {
  assert.deepEqual(filterLibraryEntries(entries).map((entry: { id: string }) => entry.id), ["metric-card", "button", "governance-pipeline"]);
});
