...
async function configureRecipe(root: string, sourceId = "portfolio-workbook") {
...
function sourcePolicy(id: string, mode: "manual" | "live-local") {
  return {
    schemaVersion: 1,
    id,
    mode,
...
async function run(workspaceRoot: string, args: string[]) {
  let stdout = "";
  let stderr = "";
