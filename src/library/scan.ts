...
import { loadWorkspaceConfig } from "../workspace/load-config.js";
import {
  MANIFEST_SPECS,
  expectedPlacement,
  manifestSpecForRoot,
...
import { scanArtifactLocalResources } from "./local-resources.js";
import type {
  LibraryDiagnostic,
  LibraryEntry,
  LibraryReference,
...
export interface ScanWorkspaceLibraryResult {
...
export async function scanWorkspaceLibrary(
  workspaceRoot: string,
): Promise<LibraryScan> {
  const canonicalWorkspaceRoot = await realpath(resolve(workspaceRoot));
  const config = await loadWorkspaceConfig(canonicalWorkspaceRoot) as unknown as { libraryRoots: Record<string, string> };
  const entries: LibraryEntry[] = [];
  const issues: LibraryDiagnostic[] = [];
  ...
  resolveReferences(entries, references, issues as unknown as Parameters<typeof resolveReferences>[2]);

  const diagnostics: LibraryDiagnostic[] = issues as LibraryDiagnostic[];
...
export interface FindLibraryEntriesOptions {
  kind?: string;
  level?: string;
  collection?: string;
}

export function findLibraryEntries(
  entries: readonly LibraryEntry[],
  filters: FindLibraryEntriesOptions = {},
): LibraryEntry[] {
...
async function readManifestEntry(context: {
  ...
  if (
    manifest.kind &&
    !context.spec.expectedKinds.includes(manifest.kind as string)
  ) {
    context.issues.push(
      issue(
        "error",
  ...
  const level = (manifest.level as string | null | undefined) ?? null;
  const lifecycle: "Local" | "Collection" | "Core" =
    (manifest.lifecycle as "Local" | "Collection" | "Core" | undefined) ??
    (level === "core" ? "Core" : level === "collection" ? "Collection" : "Local");
  const scope: "local" | "collection" | "core" =
    (manifest.scope as "local" | "collection" | "core" | undefined) ??
    (level === "core" ? "core" : level === "collection" ? "collection" : "local");
  ...
  return {
    ...
    kind:
      typeof manifest.kind === "string"
        ? manifest.kind
        : (context.spec.expectedKinds[0] as string),
    category: context.spec.category,
    title: (manifest.title as string | undefined) ?? (manifest.name as string | undefined) ?? id,
    level,
    collection: (manifest.collection as string | null | undefined) ?? null,
    ownerArtifact: (manifest.ownerArtifact as string | null | undefined) ?? null,
    user:
      context.spec.category === "artifact"
        ? (manifest.user as string | null | undefined) ?? null
        : null,
    rootKey: context.spec.rootKey,
...
function summarise(
  entries: readonly LibraryEntry[],
  issues: readonly LibraryDiagnostic[],
): ScanSummary {
...
function issue(
  severity: LibraryDiagnostic["severity"],
  code: string,
  message: string,
...
function compareIssues(
  left: LibraryDiagnostic,
  right: LibraryDiagnostic,
): number {
  const severityOrder = { error: 0, warning: 1 };

  return (
    (severityOrder[left.severity as keyof typeof severityOrder] ?? 9) -
      (severityOrder[right.severity as keyof typeof severityOrder] ?? 9) ||
