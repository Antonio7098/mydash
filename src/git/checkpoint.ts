...
import { getRepositoryStatus, type RepositoryStatus } from "./status.js";
import {
...
import { validateWorkspace, type WorkspaceValidationReport } from "../validation/workspace-validation.js";
import { scanWorkspaceLibrary } from "../library/scan.js";
import type { LibraryScan } from "../library/types.js";
import { analyseCheckpointImpact, type CheckpointImpactReport } from "./impact.js";
...
export interface CheckpointPushResult {
  requested: boolean;
  pushed: boolean;
  target: string | null;
...
export interface CheckpointReport {
  ...
  paths: string[];
  selectedChanges: ChangeStatus[];
  validation: WorkspaceValidationReport;
  impact: CheckpointImpactReport;
  commit: CheckpointCommitResult | null;
...
export async function checkpointWorkspace(
  ...
  const statusBefore = await getRepositoryStatus(options.workspaceRoot);
  assertCheckpointable(statusBefore);

  const paths = await normaliseCheckpointPaths(
    statusBefore.root,
    options.paths,
  );
  const selectedChanges = changesForPaths(statusBefore.root, paths);
  ...
  const fingerprintBefore = await fingerprintChanges(
    statusBefore.root,
    selectedChanges,
  );
  const validation = await validateWorkspace({
    ...
    minify: false,
    maxBytes: options.maxBytes ?? DEFAULT_MAX_EXPORT_BYTES,
  ...
  const scan: LibraryScan = await scanWorkspaceLibrary(statusBefore.root);
  const changedPaths = uniqueChangedPaths(selectedChanges);
  const impact = analyseCheckpointImpact(scan, changedPaths);

  if (impact.summary.requiresAcknowledgement && !options.acknowledgeImpact) {
    throw new GitSafetyError(
  ...
  const changesAfterValidation = changesForPaths(statusBefore.root, paths);
  const fingerprintAfter = await fingerprintChanges(
    statusBefore.root,
  ...
  if (
    ...
    !sameChangeSet(selectedChanges, changesAfterValidation)
  ) {
    throw new GitSafetyError(
  ...
  runGit(["add", "-A", "--", ...paths], { cwd: statusBefore.root });
  ...
  const commit: CheckpointCommitResult = {
    hash: runGit(["rev-parse", "--short", "HEAD"], { cwd: statusBefore.root }).stdout.trim(),
    fullHash: runGit(["rev-parse", "HEAD"], { cwd: statusBefore.root }).stdout.trim(),
  ...
  const push = options.push === false
    ...
    : pushCurrentBranch(statusBefore.root);
  const warnings: { code: string; message: string }[] = [];

  if (push.obstacle) {
    warnings.push({
      code: push.code ?? "PUSH_OBSTACLE",
  ...
  return {
    ...
    exitCode:
      push.requested && !push.pushed && push.code !== "NO_PUSH_TARGET" ? 1 : 0,
    dryRun: false,
    repository: statusBefore,
    paths,
    selectedChanges,
...
function pushCurrentBranch(root: string): CheckpointPushResult {
  const branch = runGit(["branch", "--show-current"], { cwd: root }).stdout.trim();
  ...
  if (upstream.status === 0) {
    ...
    const result = runGit(["push"], { cwd: root, allowFailure: true });
  ...
  const remotes = runGit(["remote"], { cwd: root, allowFailure: true }).stdout
    .split(/\r?\n/)
    .map((value) => value.trim())
    .filter(Boolean);
  ...
  const result = runGit(["push", "-u", "origin", branch], {
    cwd: root,
    allowFailure: true,
  });
...
function assertCheckpointable(status: RepositoryStatus): void {
  if (!status.head) {
    throw new GitSafetyError(
...
function uniqueChangedPaths(changes: ChangeStatus[]): string[] {
  return [
    ...new Set(
      changes.flatMap((change) => [change.path, change.originalPath].filter(Boolean) as string[]),
    ),
  ];
}

function sameChangeSet(left: ChangeStatus[], right: ChangeStatus[]): boolean {
  if (left.length !== right.length) return false;

  const leftKeys = new Set(left.map((change) => `${change.status}|${change.path}|${change.originalPath}`));
  for (const change of right) {
    if (!leftKeys.has(`${change.status}|${change.path}|${change.originalPath}`)) return false;
  }
  return true;
}